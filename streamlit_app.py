    ...
    try:
        ...
        for k in range(len(ligne)):
            ...
            if cle_legende not in dictionnaire_legende:
                ...
                continue  # ✅ CORRIGÉ : était "continuelements_assembles" (typo)
...
def changer_semaine(sens):
    if "semaine" in st.session_state:
        ...
        if 1 <= nouvelle_semaine <= 30:
            st.session_state.semaine = str(nouvelle_semaine)  # ✅ CORRIGÉ : stocké en str
...
def gerer_outils_debug(classe_selectionnee, annee_scolaire_actuelle):
    ...
    if st.button("Recharger les données (Colloscope/Légende)", key="reload_data_btn_debug"):
        st.cache_data.clear()
        st.success("Cache des données vidé.")
        st.rerun()
...
def principal():
    ...
    with main_tabs[0]:
        ...
        st.session_state.groupe = st.sidebar.text_input(
            "Groupe", value=st.session_state.groupe, key="groupe_input"
        )

        # ✅ CORRIGÉ : utilise semaine_saved pour conserver la semaine choisie
        try:
            semaine_index_default = int(st.session_state.semaine) - 1
        ...
        st.session_state.semaine = st.sidebar.selectbox(
            "Semaine", options=[str(i) for i in range(1, 31)],
            index=semaine_index_default,
            key="semaine_select"
        )
        ...
        if cols[1].button("◀", key="prev_semaine_btn"):
            changer_semaine(-1)
            st.sidebar.info("Veuillez vérifier votre colloscope papier pour éviter les erreurs.", icon="⚠️")
            st.rerun()  # ✅ CORRIGÉ : rerun pour mettre à jour le selectbox
